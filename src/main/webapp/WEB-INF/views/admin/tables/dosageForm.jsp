...
<div class="col-md-12 admin-grid">
    <div class="panel pb10 panel-widget" id="p05">
        <div class="panel-heading">
            <span class="panel-title"> DosageForms </span>
            <button id="deleteBtn" onclick="deleteDosageForms()" class="btn btn-warning pull-right" style="margin-top:15px;margin-right: 10px;"><i class="fa fa-trash"></i> Delete </button>
            ...
            <button id="saveBtn" onclick="saveDosageForms()" class="btn btn-default pull-right" style="margin-top:15px;margin-right: 10px;" disabled><i class="fa fa-plus"></i> Save </button>
            <button id="editBtn" onclick="updateDosageForms()" class="btn btn-default pull-right" style="margin-top:15px;margin-right: 10px;" disabled><i class="fa fa-pencil"></i> Update </button>
        </div>
        <div class="panel-body p15">
            <div class="table-responsive">
                <table class="table table-striped table-hover" id="dosageFormTable" width="100%">
                    <thead>
                        <tr>
                            <th>Select</th>
                            <th>Name</th>
...
<script>
    ...
    function addForm () {
       ...
       $('#dosageFormTable tbody').prepend(html);
        $('#saveBtn').prop('disabled', false);
        count++;
    }

    function saveDosageForms () {
        var formContent = {};
        formContent.dosageFormList=[];
        for(var idx = 0;idx < count;idx++) {
            if($('#addName'+idx).length != 0 && $('#addEnabled'+idx).length != 0) {
                var enabled = '';
                if($('#addName'+idx).val() != '') {
                    if($("#addEnabled"+idx).is(':checked')) {enabled = true;} else {enabled = false;}
                    formContent.dosageFormList.push({name:$('#addName'+idx).val(),enabled:enabled});
                }
            }
        }

        if(formContent.dosageFormList.length > 0 ) {
            passData('/dosage_form','POST',formContent,refresh);
        }
    }
    ...
    callGet('/dosage_form',dosageFormTable);

    function deleteDosageForms() {
        var url = '/dosage_form?ids='+selectedIds[0];
    ...
    function updateDosageForms() {
        ...
        for(var idx =0;idx < selectedIds.length;idx++) {
            ...
            if($('#name'+idx).val() != '') {
                ...
                formContent.dosageFormList.push({id:selectedIds[idx],name:$('#name'+selectedIds[idx]).val(),enabled:enabled});
            }
        }

        if(formContent.dosageFormList.length > 0 ) {
            passData('/dosage_form','PUT',formContent,refresh);
        }
    }
